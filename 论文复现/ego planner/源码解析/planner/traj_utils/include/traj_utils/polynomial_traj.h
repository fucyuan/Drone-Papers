...
public:
  ...
  void init()
  {
      num_seg = times.size(); // 设置 num_seg 为 times 向量的大小，即时间段的数量
      time_sum = 0.0; // 初始化总时间为0

      // 遍历 times 向量，累加每个时间段，计算总时间
      for (int i = 0; i < times.size(); ++i)
      {
          time_sum += times[i]; // 累加每个时间段到总时间
      }
  }
  ...
  /*特定时间 t 的位置。
  它首先确定给定时间 t 属于哪个轨迹段，
  然后根据该段的多项式系数（存储在 cxs、cys、czs 中）和时间 t
   计算出轨迹在此时刻的具体位置*/
{
    // 确定段数
    int idx = 0;
    ...
    // 评估过程
    int order = cxs[idx].size(); // 获取当前段的多项式阶数
    Eigen::VectorXd cx(order), cy(order), cz(order), tv(order); // 分别为x, y, z坐标和时间向量声明变量

    // 将多项式系数赋值给cx, cy, cz，并计算时间向量tv
    for (int i = 0; i < order; ++i)
    {
        cx(i) = cxs[idx][i], cy(i) = cys[idx][i], cz(i) = czs[idx][i]; // 为每个坐标轴赋值系数
        tv(order - 1 - i) = std::pow(t, double(i)); // 计算时间向量的各元素
    }

    Eigen::Vector3d pt; // 声明用于存储点坐标的变量
    pt(0) = tv.dot(cx), pt(1) = tv.dot(cy), pt(2) = tv.dot(cz); // 计算每个坐标轴上的位置并赋值
    return pt; // 返回计算得到的点
}
